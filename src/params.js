...
export const GROUPS = [
  ...
  },
  {
    key: 'look',
    fields: [
      { key: 'style', label: 'style', type: 'choice', choices: ['ink', 'grid'], def: 'grid' },
      ...
      { key: 'gridMark', label: 'mark', type: 'choice', choices: ['square', 'cross', 'ascii'], def: 'square', when: (p) => p.style === 'grid' },
      { key: 'gridCell', label: 'cell', type: 'range', min: 3, max: 40, step: 1, def: 6, unit: 'px', when: (p) => p.style === 'grid' },
      { key: 'gridInset', label: 'inset', type: 'range', min: 0.2, max: 1, step: 0.05, def: 0.55, when: (p) => p.style === 'grid' && p.gridMark !== 'ascii' },
      { key: 'gridLevels', label: 'levels', type: 'range', min: 2, max: 9, step: 1, def: 5, when: (p) => p.style === 'grid' && p.gridMark !== 'square' },
